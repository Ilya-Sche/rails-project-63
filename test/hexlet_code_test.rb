...
class HexletCodeTest < Minitest::Test
  ...
  def test_form_for_input_name_job_as_text
    ...
    assert_equal "<form action='#' method='post'><input name='name' type='text' value='rob'>" \
                 "<textarea name='job' cols='10' rows='20'>hexlet</textarea></form>", result
  end
  ...
  def test_form_for_input_name_additional_attribute
    ...
    assert_equal "<form action='#' method='post'><input name='name' type='text' value='rob' class='user-input'>" \
                 "<input name='job' type='text' value='hexlet'></form>", result
  end
